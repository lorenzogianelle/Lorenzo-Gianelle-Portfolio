#In this R file the simulation has been implemented using the tested distribution,
#detail on how the distributions are selected are available in the report.

#more detailed commenting on how the code is structured ara avaiable in the code
#named BoxCar.R
...
while (tnow < termination.event$event.time){ # right now tnow is previous time
  ...
  if(event.type.now == "DA"){
    ...
    rr.1 <- list(AB = abandon, WT = mean_waiting_time, IPH = mean_income_hour, FAIR_SD = sd_income_hour, FAIR_Q = quan ,RES = c(mean_rest_time,sd_rest_time),
                 RES_Q = quan_res)
  }
